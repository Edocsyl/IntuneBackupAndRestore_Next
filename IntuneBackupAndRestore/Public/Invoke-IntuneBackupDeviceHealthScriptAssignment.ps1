function Invoke-IntuneBackupDeviceHealthScriptAssignment {
    ...
    $healthScripts = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceManagement/deviceHealthScripts" | Select-Object -ExpandProperty Value

    foreach ($healthScript in $healthScripts) {
        $assignments = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/beta/deviceManagement/deviceHealthScripts/$($healthScript.id)/assignments"
