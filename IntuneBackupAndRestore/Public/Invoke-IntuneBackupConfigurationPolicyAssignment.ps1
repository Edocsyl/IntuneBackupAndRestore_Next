function Invoke-IntuneBackupConfigurationPolicyAssignment {
    ...
    foreach ($configurationPolicy in $configurationPolicies) {
        $assignments = Invoke-GraphRequest -Method GET -Uri "https://graph.microsoft.com/beta/deviceManagement/configurationPolicies/$($configurationPolicy.id)/assignments" -OutputType JSON | ConvertFrom-Json
