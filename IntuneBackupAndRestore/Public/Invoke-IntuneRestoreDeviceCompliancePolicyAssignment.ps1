function Invoke-IntuneRestoreDeviceCompliancePolicyAssignment {
    ...
    foreach ($deviceCompliancePolicy in $deviceCompliancePolicies) {
        ...
        try {
            if ($restoreById) {
                $deviceCompliancePolicyObject = (Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/beta/deviceManagement/deviceCompliancePolicies/$deviceCompliancePolicyId").value
            }
            else {
        ...
        try {
            $null = Invoke-MgGraphRequest -Method POST -Body $requestBody.toString() -URI "https://graph.microsoft.com/beta/deviceManagement/deviceCompliancePolicies/$($deviceCompliancePolicyObject.id)/assign" -ErrorAction Stop
            [PSCustomObject]@{
                "Action" = "Restore"
